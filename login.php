...
<form action="login.php" method="post">
  ...
  <label for=""></label>
  <input type="text" name="username" id="username" placeholder="Username" class="text">
...
<?php
	...
	$password=$_POST['password'];
		$house=mysql_query("SELECT House FROM userinfo WHERE Username='".$username."' AND Password='".$password."';",$conn);
		if($house!='')
		{	
				if($house==Gryffindor)
				{
					header('location:gryffindor.html');
				}
				else if($house==Slytherin)
				{
					header('location:slytherin.html');
				}
				else if($house==Ravenclaw)
				{
					header('location:ravenclaw.html');
				}
				else
				{
					header('location:hufflepuff.html');
				}
		}
